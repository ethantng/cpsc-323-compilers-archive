...
/* ignore
-- R14 - R16 has INDIRECT LEFT RECURSION (<Statement> --> <Compound> --> <Statement List> --> <Statement>) --
    SL -> S | S SL
    S -> C | A | IF | RE | PR | SC | WH
    C -> { SL }
    ...
    { 
        C | A | IF | RE | PR | SC | WH
        | 
        C SL | A SL | IF SL | RE SL | PR SL | SC SL | WH SL
    }
    ...
    C -> 
        { 
            A C' | IF C' | RE C' | PR C' | SC C' | WH C' 
            |
            A SL C' | IF SL C' | RE SL C' | PR SL C' | SC SL C' | WH SL C' 
        }
    C' -> C C' | C SL C' | epsilon  
*/